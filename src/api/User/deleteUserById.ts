...
export type DeleteUserByIdResponse = HttpResponse<DeleteUserByIdDto>;
...
export const deleteUserById = (
  ...
  if (!params) {
    throw new Error('【deleteUserById】请求参数为必填项，禁止传入undefined/null！');
  }
  ...
  const replacePlaceholder = (url: string, params: any) => {
    return url.replace(/\{(\w+)\}/g, (_, key) => {
      if (params[key] === undefined || params[key] === null) {
        throw new Error(`【deleteUserById】路径参数${key}未传入，请检查params！`);
      }
  ...
  return deleteRequest<DeleteUserByIdDto>(targetUrl, params, requestConfig);
};
