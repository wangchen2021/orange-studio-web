...
export type GetUserByIdResponse = HttpResponse<GetUserByIdDto>;
...
export const getUserById = (
  ...
  if (!params) {
    throw new Error('【getUserById】请求参数为必填项，禁止传入undefined/null！');
  }
  ...
  const replacePlaceholder = (url: string, params: any) => {
    return url.replace(/\{(\w+)\}/g, (_, key) => {
      if (params[key] === undefined || params[key] === null) {
        throw new Error(`【getUserById】路径参数${key}未传入，请检查params！`);
      }
  ...
  return getRequest<GetUserByIdDto>(targetUrl, params, requestConfig);
};
